 ...
 * 
 *  TODO
 * @author Claude Anderson and (Zesun Yang). Created Oct 12, 2011.
 */
public class AdderMain {

	// I pass this sum to a button listener that keep track of the sum on the calculator
	...
	public static void main(String[] args) {
		...
		ButtonListener aL=new ButtonListener(display,String.valueOf(sum));
		...
		for (int row = 0; row < 3; row++) {
			for (int col = 0; col < 3; col++) {
				...
				// I added a button listener that taks in the current display panel of 
				// the calculator as well as the actual sum 
				b.setFont(f);
				buttonList.add(b);
				buttonPanel.add(b);
			}
		...
		for (String s : remainingButtonLabels) {
			...
			//same as comments on line 61-62
			b.setFont(f);
			buttonList.add(b);
			buttonPanel.add(b);
		...
		// Done:  Add the rest of the code needed to get a working "Adding calculator":
